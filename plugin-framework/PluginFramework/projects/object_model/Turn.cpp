...
void move_(C_TurnHandle handle, int x, int y)
{
	ITurn* t = reinterpret_cast<Turn*>(handle);
	t->move(x, y);
}

void attack_(C_TurnHandle handle, int id)
{
	ITurn* t = reinterpret_cast<Turn*>(handle);
	t->attack(id);
}
...
static void doSingleFightSequence(ActorInfo& attacker, ActorInfo& defender)
{
	...
	// Deal damage
	int damage = 1 + rand() % attacker.damage;
